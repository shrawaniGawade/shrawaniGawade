...
The hero name fades into place once. The user-supplied interests — Curious Mind, Explainer and Automation Recorder — then reveal in sequence with a matching icon and a drawn gold underline. Each phrase has five seconds within a fifteen-second loop. The user's preferred line, “Developer, with a curious mind.”, remains visible beneath the interests. Portrait geometry and contribution data remain stationary. All motion follows `prefers-reduced-motion`: with that preference enabled, all three interests appear together, flowers remain open, the shield and lock stay visible, and DeepAttend shows its completed checkmark. The README’s links provide interaction through the clickable project and contact cards. There are no JavaScript or hover-dependent controls inside the SVG images.

The contribution garden has a slim, muted green snake with a tapered tail and rounded turns. Short, layered SVG strokes form its body; a native motion path carries the head along the same curve. A shared timeline keeps them aligned at a steady pace, with gentle fades between calendar bands. V and S shapes remain hidden in the route; neither letter nor the full route is drawn as a separate graphic. Each 18-week band receives one visit per loop. The calendar uses compact date labels, a clear contribution total and a single footer row. Contribution cells keep their real dates, counts and colors throughout. The daily refresh generates the animation with the calendar, using only native SVG and CSS. Reduced motion hides the decorative snake and leaves the complete calendar visible.
